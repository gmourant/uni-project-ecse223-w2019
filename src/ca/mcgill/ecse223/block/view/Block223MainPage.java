...
public class Block223MainPage extends JFrame {
	...
	public final static ViewTheme THEMES[] = {
			...
			new ViewTheme("Spring Galore", new Color(206, 236, 151), new Color(255, 202, 212), new Color(217, 187, 249),
					...
					new Color (152, 93, 204), new Color(247,225,215), new Color(78, 199, 206),
	...
	private void displayPage(Page toDisplay) {
		...
		if (toDisplay != Page.login && toDisplay != Page.signUp && toDisplay != Page.welcome
				&& toDisplay != Page.playGame && toDisplay != Page.chooseGame) {
		...
		if (toDisplay != Page.login && toDisplay != Page.signUp && toDisplay != Page.welcome
				&& toDisplay != Page.chooseGame) { // && check if is not published
			try {
	...
	private void setupTopMenu() {
		...
		JPanel exitMin = new JPanelWithBackground(JPanelWithBackground.Background.header,
...
//            }
	...
	public void setCurrentGameDisplay(String txt) {
		if (currentGameDisplay == null || txt.equals(""))
			return;
