...
public class Block223Controller {
	 ...
	 *                               AdminRole
	 * @throws InvalidInputException If the user is not the admin who created the
	 *                               game
	...
	public static void setGameDetails(int nrLevels, int nrBlocksPerLevel, int minBallSpeedX, int minBallSpeedY,
		...
		if (nrBlocksPerLevel < game.numberOfBlockAssignments()) // 1 on this line is a placeholder for the existing
	...
	/**
	 * This method takes finds a game, and sets it as the currently played game in
	 * Block223Application.
	 * 
	 * @author Kelly Ma
	 ...
	 * @throws InvalidInputException If the user if the points are not between 1 and
	 *                               1000
	 */
	...
	/**
	 *
	 * This method updates a block with new values. It requires a block ID, color
	 * values (RGB) and the point value of the block.
	 * 
	...
	/**
	 *
	 * This method assigns a block to a position in a game's level. It needs a level
	 * index, a block ID and a x/y grid position.
	 * 
	 * @author Mathieu Bissonnette
	 *
	 * @param id                     The ID of the desired block.
	 * @param level                  The index of the desired level.
	 * @param gridHorizontalPosition The grid horizontal position where the block
	 *                               will be positioned.
	 * @param gridVerticalPosition   The grid vertical position where the block will
	 *                               be positioned.
	 *
	...
	public static void positionBlock(int id, int level, int gridHorizontalPosition, int gridVerticalPosition)
		...
		if (game.getNrBlocksPerLevel() == game.numberOfBlocks())
			throw new InvalidInputException("The number of blocks has reached the maximum number ("
					+ Block223Application.getCurrentGame().getNrBlocksPerLevel() + ") allowed for this game.");
		...
		try {
			...
			if (e.getMessage().equals("X out of bounds.")) {
				throw new InvalidInputException(
						"The horizontal position must be between 1 and " + Game.GRID_DIMENSIONS + ".");
			} else {
				throw new InvalidInputException(
						"The vertical position must be between 1 and " + Game.GRID_DIMENSIONS + ".");
			}
		}
	 ...
	 * 
	 * @author Sofia Dieguez This method does not take any arguments.
	 ...
	 * @throws InvalidInputException :If the currentUserRole is not the admin
	 *                               associated to this specific game
	 * @throws InvalidInputException :If saving the root class throws its own
	 *                               RuntimeException
	 */
	 ...
	 * @throws InvalidInputException :If playerPassword and adminPassword are the
	 *                               same
	 * @throws InvalidInputException :If playerPassword is null or empty
	 * @throws InvalidInputException :If the username provided already exists (it is
	 *                               not unique)
	 ...
	 * @throws InvalidInputException :If a UserRole with password does not exist for
	 *                               the user
	 */
	...
	/**
	 * logout method: log user out of the game session This method does not take any
	 * arguments. This method does not throw any InvalidInputExceptions
	 */
	...
	public static void moveBlock(int level, int oldGridHorizontalPosition, int oldGridVerticalPosition,
		...
		BlockAssignment assignment = selectedLevel.findBlockAssignment(oldGridHorizontalPosition,
		...
		BlockAssignment assignmentTaken = selectedLevel.findBlockAssignment(newGridHorizontalPosition,
		...
		// set the new horizontal position for the block and check if the position is
		// available
		...
		// set the new vertical position for the block and check if the position is
		// available
	 ...
	 *                               be positioned.
	 *
	 * @throws InvalidInputException if the user isn't the admin that created the
	 *                               game.
	 ...
	 * @throws InvalidInputException If the game does not exist.
	 * @throws InvalidInputException If the player is not the one who started the
	 *                               game.
	 */
	...
	/**
	 * testGame method allows an admin to test their game before officially
	 * publishing it.
	 * 
	 * @author Sofia Dieguez
	 * @param Block223PlayModeInterface user interface
	 * @throws InvalidInputException if the currentUserRole isn't set to an admin
	 *                               role.
	 * @throws InvalidInputException if a game hasn't been selected yet.
	 * @throws InvalidInputException if the admin who wants to test the game wasn't
	 *                               the game's creator.
	 */
	...
	/**
	 * publishGame method allows an admin officially publish their game for players
	 * to play it.
	 * 
	 * @author Sofia Dieguez This method does not take any arguments.
	 * @throws InvalidInputException if the currentUserRole isn't set to an admin
	 ...
	 * @throws InvalidInputException if the admin who wants to test the game wasn't
	 *                               the game's creator.
	 * @throws InvalidInputException if there isn't at least one block defined in
	 *                               the game
	 */
	...
	/**
	 * This method returns a list of designable games for the current admin. Author:
	 * Georges Mourant
	 *
	...
	public static List<TOGame> getDesignableGames() throws InvalidInputException {
		...
		for (Game game : games) {
			...
			if (gameAdmin.equals(admin) && !game.isPublished()) {
				TOGame to = new TOGame(game.getName(), game.numberOfLevels(), game.getNrBlocksPerLevel(),
	...
	public static List<TOGridCell> getBlocksAtLevelOfCurrentDesignableGame(int level) throws InvalidInputException {
		...
		for (BlockAssignment assignment : assignments) {
			...
			TOGridCell cell = new TOGridCell(assignment.getGridHorizontalPosition(),
					assignment.getGridVerticalPosition(), block.getId(), block.getRed(), block.getGreen(),
					block.getBlue(), block.getPoints());
			result.add(cell);
		}
	...
	/**
	 * @author Kelly Ma
	 * @return A transfer object containing all information for a currently played
	 *         game
	 * @throws InvalidInputException If the UserRole is not set
	 * @throws InvalidInputException If a PlayedGame is not selected
	 * @throws InvalidInputException If the currentUserRole is an AdminRole, but the
	 *                               currentPlayer is a PlayerRole
	 * @throws InvalidInputException If the currentAdmin is the not the game's admin
	 * @throws InvalidInputException If the currentUserRole is a player, but the
	 *                               player is not set
	 */
	public static TOCurrentlyPlayedGame getCurrentPlayableGame() throws InvalidInputException {
		...
		if (currentUserRole == null)
			throw new // Verifies that the user role is set
		...
		if (pgame == null)
			throw new InvalidInputException("A game must be selected to play it."); // Throws exception if no game set

		Player currentPlayer = pgame.getPlayer();
		if (currentUserRole instanceof Admin && currentPlayer != null)
			throw new // Verifies that the user role is the player themself
			InvalidInputException("Player privileges are required to play a game.");

		Admin gameAdmin = pgame.getGame().getAdmin();
		if (currentUserRole instanceof Admin && currentUserRole != gameAdmin)
			throw new // Verifies the current admin is the one who created the game
		...
		// Create a boolean to check the status of the game (whether or not we should
		// play)
		boolean paused = pgame.getPlayStatus() == PlayStatus.Ready || pgame.getPlayStatus() == PlayStatus.Paused; // Checks
																													// if
																													// game
																													// is
																													// paused
	...
	public static TOHallOfFame getHallOfFame(int start, int end) throws InvalidInputException {
		...
		if (!(Block223Application.getCurrentUserRole() instanceof Player))
			throw new // Verifies that the user is a Player
			InvalidInputException("Player privileges are required to access a game's hall of fame.");
		PlayedGame pgame = Block223Application.getCurrentPlayableGame(); // Obtain current played game
		if (pgame == null)
			throw new InvalidInputException("A game must be selected to view its hall of fame."); // Throws exception if
																									// no game set
		...
		if (start < 1)
			start = 1; // Ensure that start index is >= 1
		if (end > game.numberOfHallOfFameEntries())
			end = game.numberOfHallOfFameEntries(); // End cannot exceed total # of entries
		...
		for (int index = start; index >= end; index--) {
			...
			// // Old method to find username
			new TOHallOfFameEntry(index + 1, game.getHallOfFameEntry(index).getPlayername(),
					game.getHallOfFameEntry(index).getScore(), result); // Create transfer object
	...
	public static TOHallOfFame getHallOfFameWithMostRecentEntry(int numberOfEntries) throws InvalidInputException {
		...
		for (int index = start; index >= end; index--) {
			// String username =
			// User.findUsername(game.getHallOfFameEntry(index).getPlayer());
			new TOHallOfFameEntry(index + 1, game.getHallOfFameEntry(index).getPlayername(),
					game.getHallOfFameEntry(index).getScore(), result); // Create transfer object
		}
	 ...
	 * @author Kelly Ma
	 * @author Imane Chafi
	 * @param direction Describes left, right, or pause Assume that all other checks
	...
	/**
	 * This method finds a block inside a list of blocks depending on its ID. Author
	 * : Imane Chafi
	 */
	...
	private static BlockAssignment findBlockAssignment(Level theLevel, int gridHorizontalPosition,
	 ...
	 * @throws InvalidInputException If no game is selected.
	 * 
	 * @throws InvalidInputException If the user is not the admin that created the
	 *                               game.
	 * 
